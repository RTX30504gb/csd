...
class RiskScore(Base):
    """The computed risk score for a token (Phase 13/18).

    Stores the final score, the breakdown by category, and the reasons.
    The 'outcome' field is populated later (ground truth) for ML training.
    """
    ...
    level: Mapped[str] = mapped_column(String(16), nullable=False)  # Low, Suspicious, High, Critical
    ...
    outcome: Mapped[str | None] = mapped_column(String(16), nullable=True)  # rug | safe | unknown

    __table_args__ = (
        UniqueConstraint("token_address", "computed_at", name="uq_token_score_time"),
    )


class RiskEvent(Base):
    """A specific on-chain event that triggered a score update (Phase 13).

    Example: A sudden 50% liquidity removal.
    """
    ...
    kind: Mapped[str] = mapped_column(String(32), nullable=False)  # e.g. 'liquidity_removal'
